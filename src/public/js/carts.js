...
const deleteFromCart = async (productId) => {
  ...
  try {
    await axios.delete(`${url}/api/carts/${cartId}/product/${productId}`);
...
const substractFunction = async (productId, quantity) => {
  ...
  try {
    await axios.put(`${url}/api/carts/${cartId}/product/${productId}`, data);
    socket.emit('restarProducto', JSON.stringify(dataSocket))
  } catch (error) {
    console.error(error);
  }
...
const sumFunction = async (productId, quantity) => {
  ...
  try {
    const response = await axios.put(`${url}/api/carts/${cartId}/product/${productId}`, data);
...
async () => {
  try {
    ...
    await axios.delete(`${url}/api/carts/${cartId}`)
...
async () => {
  ...
  try {
    ...
    const response = await axios.get(`${url}/api/carts/${cartId}`)
    cart = response.data
    console.log("Este es el cart", cart)
    
    if (cart) {
      const productsToPurchase = cart.products
      const responsePurchase = await axios.post(`${url}/api/carts/${cartId}/purchase`, { products: productsToPurchase })
      ...
      if(responsePurchase.status === 200) {
        await axios.post(`${url}/api/mail`, body, {header: headers})
        await axios.get(`${url}/api/sms/${body.userId}/${body.ticket.code}`, body, {header: headers})
      }
